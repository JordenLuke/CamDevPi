...
static struct mt9m034_priv *to_mt9m034(const int fd)
{
...
{
	return wiringPiI2CWriteReg16(fd, (int) addr)
}
 ...
 *
 */
static int mt9m034_sequencer_settings(int fd)
{
	int i, ret;
	MT9M034_WRITE(ret, fd, MT9M034_SEQ_CTRL_PORT, 0x8000)

	for(i = 0; i < ARRAY_SIZE(mt9m034_seq_data); i++){
		MT9M034_WRITE(ret, fd, MT9M034_SEQ_DATA_PORT, mt9m034_seq_data[i])
	}

	MT9M034_WRITE(ret, fd, MT9M034_ERS_PROG_START_ADDR, 0x0186)
 ...
 *
 */
static int mt9m034_linear_mode_setup(int fd)
{
	int ret;

	/* Disable Streaming */
	MT9M034_WRITE(ret, fd, MT9M034_RESET_REG, MT9M034_STREAM_OFF)

	/* Operation mode control */
	MT9M034_WRITE(ret, fd, MT9M034_MODE_CTRL, 0x0029)
	MT9M034_WRITE(ret, fd, MT9M034_DATA_PEDESTAL, 0x00C8)
	MT9M034_WRITE(ret, fd, MT9M034_DAC_LD_14_15, 0x0F03)
	MT9M034_WRITE(ret, fd, MT9M034_DAC_LD_18_19, 0xC005)
	MT9M034_WRITE(ret, fd, MT9M034_DAC_LD_12_13, 0x09EF)
	MT9M034_WRITE(ret, fd, MT9M034_DAC_LD_22_23, 0xA46B)
	MT9M034_WRITE(ret, fd, MT9M034_DAC_LD_20_21, 0x067D)
	MT9M034_WRITE(ret, fd, MT9M034_DAC_LD_16_17, 0x0070)
	MT9M034_WRITE(ret, fd, MT9M034_DARK_CONTROL, 0x0404)
	MT9M034_WRITE(ret, fd, MT9M034_DAC_LD_26_27, 0x8303)
	MT9M034_WRITE(ret, fd, MT9M034_DAC_LD_24_25, 0xD208)
	MT9M034_WRITE(ret, fd, MT9M034_DAC_LD_10_11, 0x00BD)
	MT9M034_WRITE(ret, fd, MT9M034_ADC_BITS_6_7, 0x6372)
	MT9M034_WRITE(ret, fd, MT9M034_ADC_BITS_4_5, 0x7253)
	MT9M034_WRITE(ret, fd, MT9M034_ADC_BITS_2_3, 0x5470)
	MT9M034_WRITE(ret, fd, MT9M034_ADC_CONFIG1, 0xC4CC)
	MT9M034_WRITE(ret, fd, MT9M034_ADC_CONFIG2, 0x8050)
	MT9M034_WRITE(ret, fd, MT9M034_DIGITAL_TEST, 0x1300)
	MT9M034_WRITE(ret, fd, MT9M034_COLUMN_CORRECTION, 0xE007)
	MT9M034_WRITE(ret, fd, MT9M034_DIGITAL_CTRL, 0x0008)
	MT9M034_WRITE(ret, fd, MT9M034_RESET_REGISTER, 0x10DC)
	MT9M034_WRITE(ret, fd, MT9M034_RESET_REGISTER, 0x10D8)
	MT9M034_WRITE(ret, fd, MT9M034_BLUE_GAIN, 0x003F)
	MT9M034_WRITE(ret, fd, MT9M034_COARSE_INTEGRATION_TIME, 0x02A0)
 ...
 *
 */
static int mt9m034_set_size(int fd, struct mt9m034_frame_size *frame)
{
	...
	if (hratio == 2) {
		if (vratio == 2) {
			MT9M034_WRITE(ret, fd, MT9M034_DIGITAL_BINNING,
	...
	MT9M034_WRITE(ret, fd, MT9M034_Y_ADDR_START, mt9m034->crop.top)
	MT9M034_WRITE(ret, fd, MT9M034_X_ADDR_START, mt9m034->crop.left)
	MT9M034_WRITE(ret, fd, MT9M034_Y_ADDR_END, mt9m034->crop.top + mt9m034->crop.height - 1)
	MT9M034_WRITE(ret, fd, MT9M034_X_ADDR_END, mt9m034->crop.left + mt9m034->crop.width - 1)
	MT9M034_WRITE(ret, fd, MT9M034_FRAME_LENGTH_LINES, mt9m034->crop.height + 37)
	MT9M034_WRITE(ret, fd, MT9M034_LINE_LENGTH_PCK, MT9M034_LLP_RECOMMENDED)
	MT9M034_WRITE(ret, fd, MT9M034_COARSE_INT_TIME, 0x01C2)
	MT9M034_WRITE(ret, fd, MT9M034_X_ODD_INC, 0x0001)
	MT9M034_WRITE(ret, fd, MT9M034_Y_ODD_INC, 0x0001)
 ...
 *
 */
static int mt9m034_pll_setup(int fd)
{
	int ret;
	int i;
	struct mt9m034_priv *mt9m034 = to_mt9m034(fd);
	...
	//dev_err(&fd->dev, "Couldn't find PLL dividers for ext_freq = %d, target_freq = %d\n",
...
#endif
	MT9M034_WRITE(ret, fd, MT9M034_VT_SYS_CLK_DIV, mt9m034->pll->p1)
	MT9M034_WRITE(ret, fd, MT9M034_VT_PIX_CLK_DIV, mt9m034->pll->p2)
	MT9M034_WRITE(ret, fd, MT9M034_PRE_PLL_CLK_DIV, mt9m034->pll->n)
	MT9M034_WRITE(ret, fd, MT9M034_PLL_MULTIPLIER, mt9m034->pll->m)

	if (mt9m034->pdata->version == MT9M034_COLOR_VERSION)
		MT9M034_WRITE(ret, fd, MT9M034_DIGITAL_TEST, 0x0000)
	else
		MT9M034_WRITE(ret, fd, MT9M034_DIGITAL_TEST, 0x0080)
...
static int mt9m034_ae_setup(int fd)
{
	int ret;

	MT9M034_WRITE(ret, fd, MT9M034_RESET_REGISTER, 0x10D8)
	MT9M034_WRITE(ret, fd, MT9M034_HDR_COMP, 0x0001)
	MT9M034_WRITE(ret, fd, MT9M034_VT_SYS_CLK_DIV, 0x0002)
	MT9M034_WRITE(ret, fd, MT9M034_VT_PIX_CLK_DIV, 0x0004)
	MT9M034_WRITE(ret, fd, MT9M034_PRE_PLL_CLK_DIV, 0x0002)
	MT9M034_WRITE(ret, fd, MT9M034_PLL_MULTIPLIER, 0x002C)
	MT9M034_WRITE(ret, fd, MT9M034_DIGITAL_TEST, 0x1300)
	MT9M034_WRITE(ret, fd, MT9M034_RESET_REGISTER, 0x10DC)
	MT9M034_WRITE(ret, fd, MT9M034_EMBEDDED_DATA_CTRL, 0x1982)
	MT9M034_WRITE(ret, fd, MT9M034_BLUE_GAIN, 0x003F)
	MT9M034_WRITE(ret, fd, MT9M034_AE_CTRL_REG, 0x001B)
	MT9M034_WRITE(ret, fd, MT9M034_AE_DCG_EXPOSURE_HIGH_REG, 0x029F)
	MT9M034_WRITE(ret, fd, MT9M034_AE_DCG_EXPOSURE_LOW_REG, 0x008C)
	MT9M034_WRITE(ret, fd, MT9M034_AE_DCG_GAIN_FACTOR_REG, 0x02C0)
	MT9M034_WRITE(ret, fd, MT9M034_AE_DCG_GAIN_FACTOR_INV_REG, 0x005B)
	MT9M034_WRITE(ret, fd, MT9M034_AE_LUMA_TARGET_REG, 0x0384)
	MT9M034_WRITE(ret, fd, MT9M034_AE_HIST_TARGET_REG, 0x1000)
	MT9M034_WRITE(ret, fd, MT9M034_AE_ALPHA_V1_REG, 0x0080)
	MT9M034_WRITE(ret, fd, MT9M034_AE_MAX_EXPOSURE_REG, 0x03DD)
	MT9M034_WRITE(ret, fd, MT9M034_AE_MIN_EXPOSURE_REG, 0x0003)

	return ret;
}

static int mt9m034_is_streaming(int fd)
{
	u16 streaming;

	streaming = mt9m034_read(fd, MT9M034_RESET_REG);
	streaming = ( (streaming >> 2) & 0x0001);

	return (streaming != 0);
}

static int mt9m034_set_autoexposure( int fd, enum v4l2_exposure_auto_type ae_mode )
{
	struct mt9m034_priv *mt9m034 = to_mt9m034(fd);
	int streaming;
	int ret;
	...
	streaming = mt9m034_is_streaming(fd);

	msleep(2);

	switch(ae_mode) {
	case V4L2_EXPOSURE_AUTO: /* Shutter and Apperture */
		dev_err(&fd->dev, "Unsupported auto-exposure mode requested: %d\n", ae_mode);
		ret = -EINVAL;
		break;
	...
	case V4L2_EXPOSURE_MANUAL:
		...
		MT9M034_WRITE(ret, fd, MT9M034_EMBEDDED_DATA_CTRL, 0x1802)
		MT9M034_WRITE(ret, fd, MT9M034_AE_CTRL_REG, 0x0000)
		if (streaming) {
			MT9M034_WRITE(ret, fd, MT9M034_RESET_REG, MT9M034_STREAM_ON)
		}
		break;
	...
	case V4L2_EXPOSURE_SHUTTER_PRIORITY:
		...
		MT9M034_WRITE(ret, fd, MT9M034_EMBEDDED_DATA_CTRL, 0x1982)
		MT9M034_WRITE(ret, fd, MT9M034_AE_CTRL_REG, 0x0013)
	...
	default:
		dev_err(&fd->dev, "Auto Exposure mode out of range: %d\n", ae_mode);
		ret = -ERANGE;
		break;
	}
	if(ret == 0)
...
static int mt9m034_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	int fd = v4l2_get_subdevdata(&mt9m034->subdev);
	u16 reg16;
	int ret = 0;

	switch (ctrl->id) {
	case V4L2_CID_EXPOSURE_AUTO:
		ret = mt9m034_set_autoexposure(fd, (enum v4l2_exposure_auto_type)ctrl->val);
		if(ret < 0)
			return ret;
		break;

	case V4L2_CID_EXPOSURE:
		MT9M034_WRITE(ret, fd, MT9M034_COARSE_INT_TIME, ctrl->val)
		MT9M034_WRITE(ret, fd, MT9M034_COARSE_INT_TIME_CB, ctrl->val)
		return ret;
		break;

	case V4L2_CID_GAIN:
		MT9M034_WRITE(ret, fd, MT9M034_GLOBAL_GAIN, ctrl->val)
		MT9M034_WRITE(ret, fd, MT9M034_GLOBAL_GAIN_CB, ctrl->val)
		return ret;
		break;

	case V4L2_CID_GAIN_GREEN1:
		MT9M034_WRITE(ret, fd, MT9M034_GREEN1_GAIN, ctrl->val)
		MT9M034_WRITE(ret, fd, MT9M034_GREEN1_GAIN_CB, ctrl->val)
		return ret;
		break;

	case V4L2_CID_GAIN_RED:
		MT9M034_WRITE(ret, fd, MT9M034_RED_GAIN, ctrl->val)
		MT9M034_WRITE(ret, fd, MT9M034_RED_GAIN_CB, ctrl->val)
		return ret;
		break;

	case V4L2_CID_GAIN_BLUE:
		MT9M034_WRITE(ret, fd, MT9M034_BLUE_GAIN, ctrl->val)
		MT9M034_WRITE(ret, fd, MT9M034_BLUE_GAIN_CB, ctrl->val)
		return ret;
		break;

	case V4L2_CID_GAIN_GREEN2:
		MT9M034_WRITE(ret, fd, MT9M034_GREEN2_GAIN, ctrl->val)
		MT9M034_WRITE(ret, fd, MT9M034_GREEN2_GAIN_CB, ctrl->val)
		return ret;
		break;

	case V4L2_CID_ANALOG_GAIN:
		reg16 = mt9m034_read(fd, MT9M034_DIGITAL_TEST);
		reg16 = ( reg16 & ~MT9M034_ANALOG_GAIN_MASK ) | 
			( ( ctrl->val << MT9M034_ANALOG_GAIN_SHIFT ) & MT9M034_ANALOG_GAIN_MASK );
		MT9M034_WRITE(ret, fd, MT9M034_DIGITAL_TEST, reg16)
		return ret;
		break;

	case V4L2_CID_HFLIP:
		reg16 = mt9m034_read(fd, MT9M034_READ_MODE);
		if (ctrl->val){
			reg16 |= 0x4000;
			MT9M034_WRITE(ret, fd, MT9M034_READ_MODE, reg16)
	...
	case V4L2_CID_TEST_PATTERN:
		if (!ctrl->val){
			MT9M034_WRITE(ret, fd, MT9M034_TEST_PATTERN, 0x0000)
		}
		MT9M034_WRITE(ret, fd, MT9M034_TEST_PATTERN, ctrl->val)
		break;
	}
	return ret;
}
...
{
	int fd = v4l2_get_subdevdata(sd);
	int data;

	reg->size = 2;
	data = mt9m034_read(fd, reg->reg);
	if (data < 0)
		return data;
				...
				struct v4l2_dbg_register *reg)
{
	...
	MT9M034_WRITE(ret, fd, reg->reg, reg->val)
	...
	*/
	if (mt9m034->power_count == !on) {
		if (on) {
				...
				ret = __mt9m034_write(fd, MT9M034_RESET_REG, MT9M034_RESET);
				if (ret < 0) {
...
static int mt9m034_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	ret = mt9m034_sequencer_settings(fd);
	if (ret < 0){
...
static int mt9m034_registered(struct v4l2_subdev *sd)
{
	...
	data = mt9m034_read(fd, MT9M034_CHIP_ID_REG);
	if (data != MT9M034_CHIP_ID) {
		while(count++ < 5){
			data = mt9m034_read(fd, MT9M034_CHIP_ID_REG);
			msleep(5);
		}
		dev_err(&fd->dev, "Aptina MT9M034 not detected, chip ID read:0x%4.4x\n",
				data);
		return -ENODEV;
	}
	dev_info(&fd->dev, "Aptina MT9M034 detected at address 0x%02x\n", fd->addr);
...
/***************************************************
		I2C driver
****************************************************/
static int mt9m034_probe(int fd,
			const struct i2c_device_id *did)
{
	struct mt9m034_platform_data *pdata = fd->dev.platform_data;
	struct i2c_adapter *adapter = to_i2c_adapter(fd->dev.parent);
	struct mt9m034_priv *mt9m034;
	int ret;
	int i;

	if (pdata == NULL) {
		dev_err(&fd->dev, "No platform data\n");
		return -EINVAL;
	}

	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_WORD_DATA)) {
		dev_warn(&fd->dev, "I2C-Adapter doesn't support I2C_FUNC_SMBUS_WORD\n");
		return -EIO;
	}

	mt9m034 = devm_kzalloc(&fd->dev, sizeof(struct mt9m034_priv),
				GFP_KERNEL);
	if (mt9m034 == NULL)
		return -ENOMEM;
	...
	if (mt9m034->ctrls.error) {
		...
		dev_err(&fd->dev, "Control initialization error: %d\n",
			ret);
		goto done;
	}

	mutex_init(&mt9m034->power_lock);
	v4l2_i2c_subdev_init(&mt9m034->subdev, fd, &mt9m034_subdev_ops);
	...
	if (ret < 0) {
		...
		dev_err(&fd->dev, "Probe failed\n");
	}

	return ret;
}

static int mt9m034_remove(int fd)
{
	struct v4l2_subdev *subdev = i2c_get_fddata(fd);
	struct mt9m034_priv *mt9m034 = to_mt9m034(fd);
